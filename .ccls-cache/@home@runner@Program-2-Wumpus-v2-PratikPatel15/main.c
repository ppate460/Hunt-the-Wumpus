...
void hazards(struct HuntWumpus *checkHarzards, int **adjArr) {
  ...
  if (checkHarzards->wumpus == room1 || checkHarzards->wumpus == room2 ||
      checkHarzards->wumpus == room3 || checkHarzards->wumpus == checkHarzards->player) {
  ...
  // hazard
  if (checkHarzards->bats1 == room1 || checkHarzards->bats1 == room2 || checkHarzards->bats1 == room3 ||
      checkHarzards->bats2 == room1 || checkHarzards->bats2 == room2 || checkHarzards->bats2 == room3) {
...
void playerMove(struct HuntWumpus *movePlayer, int **adjArr, int move, int *count) {
  ...
  // if player enters a room where any bats present, player flew into some another room by bats
...
// concept is to chech if the arrows that player is shooting is killing the wumpus or kills itself.
void shootArrows(struct HuntWumpus *checkArrow, int **adjArr, int *counter, int *countArrow) {
  ...
  // as long as the player's arrow goes through adjacent rooms, checking if any of those
  // adjacent rooms has wumpus or player itself
  if (checkArrow->numShot == adjRoom1 || checkArrow->numShot == adjRoom2 || checkArrow->numShot == adjRoom3) {
    // player shot an arrow within the same room, player instantly die, Game Over
  ...
  // check if a player shot an arrow with player in the same room, if so, player die, Game Over
  else {
    printf("Room %d is not adjacent. Arrow ricochets...\n", checkArrow->numShot);
    checkArrow->arrow = adjRoom1;
    // if the player is in the same room as it shot arrow in, player die, Game Over
...
int main(void) {
  ...
  // generating random variables and assigning into following order: pit1, pit2, wumpus, player
  ...
  while (1) {
    // passing a struct and dynamically allocated array to see if the player is adjacent to any pits or bats or a wumpus
    ...
    switch (upperCapsOfUserMove) {
      ...
      case 'R':
        ...
        scanf("%d %d %d %d %d %d %d", &huntWumpus.player, &huntWumpus.wumpus, &huntWumpus.pit1, &huntWumpus.pit2,
                                      &huntWumpus.bats1, &huntWumpus.bats2, &huntWumpus.arrow);
        break;
      ...
      printf("Cheating! Game elements are in the following rooms: \n"
              ...
              huntWumpus.player, huntWumpus.wumpus, huntWumpus.pit1, huntWumpus.pit2,
              huntWumpus.bats1, huntWumpus.bats2);
      ...
      // let player shoot arrows and sorry messages, if player doesn't have an arrow to shoot
      case 'S':
        count++;
        if (countArrow > 0) {
          printf("Enter the number of rooms (1..3) into which you want to shoot, followed by the rooms themselves: ");
          scanf("%d", &numArrow);
          // number of time checks the possibility if player hitting a wumpus or itself
        ...
        // if no arrows are avilable, sorry message pop up and say look out for an arrow somewhere
        else {
      ...
      case 'M':
        scanf("%d", &move);
        // as soon as player walks into a room where an arrow is present, congrats a player
